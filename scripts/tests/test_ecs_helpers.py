...
class TestECSHelpers(unittest.TestCase):
    ...
    def test_glob_yaml_files(self):
        ...
        self.assertEqual(
            ecs_helpers.glob_yaml_files("schemas/base.yml"), ["schemas/base.yml"]
        )
        self.assertEqual(
            ecs_helpers.glob_yaml_files(["schemas/base.yml"]), ["schemas/base.yml"]
        )
        ...
        self.assertEqual(
            set(ecs_helpers.glob_yaml_files(["schemas/base.yml", "schemas/log.yml"])),
            {"schemas/base.yml", "schemas/log.yml"},
        )
        self.assertTrue(
            set(ecs_helpers.glob_yaml_files("schemas/b*.yml")).intersection(
                {"schemas/base.yml"}
            )
            != set()
        )
        self.assertTrue(
            set(ecs_helpers.glob_yaml_files("schemas/[bl]*.yml")).intersection(
                {"schemas/base.yml", "schemas/log.yml"}
            )
            != set()
        )
        min_schema_count = 46
        self.assertTrue(
            len(ecs_helpers.glob_yaml_files(ecs_helpers.glob_yaml_files("schemas")))
            >= min_schema_count
        )
        self.assertTrue(
            len(ecs_helpers.glob_yaml_files(ecs_helpers.glob_yaml_files("schemas/")))
        ...
        self.assertTrue(
            len(
                ecs_helpers.glob_yaml_files(
                    ecs_helpers.glob_yaml_files("schemas/*.yml")
                )
            )
            >= min_schema_count
        )
        self.assertEqual(
            len(
                ecs_helpers.glob_yaml_files(
                    ecs_helpers.glob_yaml_files("schemas/*.yaml")
                )
            ),
            0,
        )
    ...
    def test_remove_top_level_false_field_sets(self):
        ...
        self.assertEqual(
            ecs_helpers.remove_top_level_reusable_false(nested_schema_original),
